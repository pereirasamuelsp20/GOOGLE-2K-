...
const getApiBase = () => {
  ...
  const LAN_IP = '192.168.29.99'; // Your Mac's local IP — update if it changes
...
const CALAMITY_OPTIONS = [
  { value: 'Earthquakes', icon: '🌍' },
  { value: 'Landslides', icon: '🏔' },
  { value: 'Power Outages', icon: '⚡' },
  { value: 'Fire', icon: '🔥' },
  { value: 'Road Blocked', icon: '🚧' },
  { value: 'Flash Floods', icon: '🌊' },
  { value: 'Car Accident', icon: '🚗' },
  { value: 'Building Collapse', icon: '🏗' },
  { value: 'Chemical Leaks', icon: '☣' },
];
...
export default function ReportIssueScreen() {
  ...
  const handleSubmit = async () => {
    if (!calamityType) {
      Alert.alert('Missing Info', 'Please select a calamity type.');
      return;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>REPORT CALAMITY</Text>
      </View>
      ...
      {activeTab === 'form' ? (
        <ScrollView style={styles.scrollArea} showsVerticalScrollIndicator={false}>
          ...
          {/* Calamity Type */}
          <Text style={[styles.sectionTitle, { color: '#CC0000' }]}>CALAMITY</Text>
          <TouchableOpacity
          ...
          >
            <Text style={{ color: calamityType ? '#fff' : '#777', fontSize: 14 }}>
              {calamityType
                ? `${getCalamityIcon(calamityType)} ${calamityType}`
                : 'SELECT CALAMITY'}
            </Text>
            ...
            <View style={styles.dropdownList}>
              {CALAMITY_OPTIONS.map((opt) => (
                <TouchableOpacity
                  ...
                  onPress={() => { setCalamityType(opt.value); setDropdownOpen(false); }}
                >
              ...
              <View key={r._id} style={styles.reportCard}>
                <View style={styles.reportCardHeader}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <Text style={{ fontSize: 20 }}>{getCalamityIcon(r.calamityType)}</Text>
